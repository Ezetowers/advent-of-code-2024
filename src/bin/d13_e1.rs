...
const BUTTON_A_TOKENS: u32 = 3;
const BUTTON_B_TOKENS: u32 = 1;
...
/*---------------------------------------------------------------------------*/
fn solve_equation(claw_machine: ClawMachine) -> u32 {
    ...
    if a > 100.0 || b > 100.0 {
        debug!("Bigger than 100 solution. Skipping");
        return 0;
    }

    // Float precision is messing or calculations. Round the values obtained and
    // validate if they match the prizes
    let int_a = a.round() as u32;
    let int_b = b.round() as u32;
    let obtained_prize_x = int_a * a_x as u32 + int_b * b_x as u32;
    let obtained_prize_y = int_a * a_y as u32 + int_b * b_y as u32;
    debug!(
        "{:?} - Int A: {} - B: {} - Obtained Prize X: {} - Obtained Prize Y: {}",
        claw_machine, int_a, int_b, obtained_prize_x, obtained_prize_y
    );
    if obtained_prize_x != prize_x as u32 || obtained_prize_y != prize_y as u32 {
        return 0;
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for line in reader.lines() {
        ...
        let v: Vec<u32> = line.split(":").collect::<Vec<_>>()[1]
            .split(",")
            .map(|x| {
                trace!("Element: {}", x);
