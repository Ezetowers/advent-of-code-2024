...
impl Vector {
    ...
    fn calculate_antinode(&self) -> Vec<Point> {
        ...
        let mut new_antinode = self.a.clone();
        while !self.point_out_of_bounds(&new_antinode) {
            antinodes.push(new_antinode);
            new_antinode.x = new_antinode.x + self.x_distance;
            new_antinode.y = new_antinode.y + self.y_distance;
        }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (key, value) in antennas_positions.iter() {
        ...
        for first_point in value.iter() {
            ...
            for second_point in value.iter() {
                ...
                for antinode in vector.calculate_antinode().iter() {
                    let x: usize = antinode.x.try_into().unwrap();
                    let y: usize = antinode.y.try_into().unwrap();
                    input[x][y] = 'X';
    ...
    for i in 0..square_len {
        debug!("Matrix: {:?}", input[i]);
    }

    info!("Day 8 - Exercise 2. Result: {}", unique_antinodes.len());
    Ok(())
}
